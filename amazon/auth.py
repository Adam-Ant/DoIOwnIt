...
class AuthenticationManager:
    def __init__(self, config_manager, session):
    ...
    def generate_challange(self, code_verifier: bytes) -> bytes:
        ...
        return base64.urlsafe_b64encode(challenge_hash.digest()).rstrip(b"=")
    ...
    def get_auth_url(self, client_id: str, challenge: bytes):
        ...
        arguments = {
            ...
            "language": "en_US",
            "marketPlaceId": consts.MARKETPLACEID,
    ...
    def register_device(self, code):
        ...
        response = self.session_manager.post(
            f"{consts.AMAZON_API}/auth/register", json=data
        )
        if not response.ok:
    ...
    def refresh_token(self):
        url = f"{consts.AMAZON_API}/auth/token"
        ...
        response = self.session_manager.post(url, json=request_data)
    ...
    def login(self):
        ...
        self.window = webview.create_window("Amazon Login", url, frameless=True)
        self.logger.info("Spawning login window")
        webview.start(self.check_page)

    def check_page(self):
        while self.window.get_current_url().find("openid.oa2.authorization_code") < 1:
            pass

        page_url = self.window.get_current_url()
